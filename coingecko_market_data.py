...
def fetch_and_save_crypto_hourly_data(crypto_ID, crypto_name, folder_path='data_cryptocurrency_rate'):
    ...
    start_date = end_date - timedelta(days=90)  # Fetching data for the last 90 days for better accuracy
    ...
    if response.status_code == 200:
        ...
        df['timedelta'] = (df['datetime'] - df['hour']).abs()
        ...
        try:
            ...
            if connection.is_connected():
                ...
                for _, row in combined_df.iterrows():
                    insert_query = """
                        INSERT INTO hourly_data (Crypto_Code, value, datetime)
                        VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE value = VALUES(value);
                    """
                    insert_values = (row['Crypto_Code'], float(row['price']), row['datetime'])
