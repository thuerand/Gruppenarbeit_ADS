...
def update_database_and_csv_with_coordinates():
    ...
    try:
        ...
        cursor.execute("SELECT Domain, hq_location FROM HQ_newagency WHERE latitude IS NULL OR longitude IS NULL")
        ...
        for index, row in df.iterrows():
            if row['Domain'] in domain_to_hq:
                if pd.isna(row['latitude']) or pd.isna(row['longitude']):
                    latitude, longitude = fetch_coordinates(api_key, domain_to_hq[row['Domain']])
...
def fetch_and_create_map():
    ...
    try:
        ...
        for (lat, lon, hq), group in grouped:
            domains_list = '<br>'.join(f"{row['Domain']}" for _, row in group.iterrows())
